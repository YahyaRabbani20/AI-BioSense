"""
Created on Tue Aug 27 17:23:57 2024
...
import seaborn as sns
              ...
              # Read the dataset##
dataset=pd.read_excel('OPG-Calibrated data.xlsx')
X=dataset.iloc[:,1]
#Two output
y=dataset.iloc[:,[2]]
...
sns.distplot(y['Wn_mean(7,6)'], label='Wn_mean(7,6)')
plt.legend()
plt.xlabel('Value')
plt.ylabel('Density')
plt.show()
...
y['Intensity_class']= y['Wn_mean(7,6)'].apply(lambda x:1 if x >1.365 else 0 )
y_output=np.asarray(y)
y_output=y_output[:,1]
plt.figure(figsize=(12,8))
sns.histplot(y['Intensity_class'], kde=True, label='Wn_mean(7,6)', color='blue')
plt.legend()
plt.show()
...
def one_hot_encode(seq, fixed_length=30): # Set the maximum length with fixed length
    ...
    mapping = dict(zip("AGCTN", range(4)))  # Add 'N' for padding if the length shorter
    seq = (seq + 'N' * fixed_length)[:fixed_length]
    seq_encoded = [mapping[char] for char in seq]
    return np.eye(4)[seq_encoded]  # One-hot encoding 
...
X_input = X_input.reshape(-1, 30, 4, 1) # reshaper to be suitable for CNN input 
...
def create_model(trial):
    num_filters1 = trial.suggest_categorical('num_filters1', [4,8,16, 32,64,128])
    ...
    dense_units = trial.suggest_categorical('dense_units', [8,16,20,32,64,128,256])
    ...
    model = Sequential()
    model.add(Conv2D(num_filters1, kernel_size=kernel_size, padding='same', input_shape=(30, 4, 1)))
...
def objective(trial):
    num_folds = trial.suggest_int('num_folds', 2,100)  #Start from at least 2
...
df.to_csv('best_hyperparametersforOPGNR_N6.csv', index=False)
